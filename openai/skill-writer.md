...
<!-- PATH CONVENTION (Claude)
  Companion files are installed to ~/.config/openai/ by running: ./claude/install.sh
    refs/        →  ~/.config/openai/refs/         (on-demand specs incl. refs/modes/)
    templates/   →  ~/.config/openai/templates/
    eval/        →  ~/.config/openai/eval/         (incl. trigger-eval.example.json)
    optimize/    →  ~/.config/openai/optimize/
    scripts/     →  ~/.config/openai/scripts/      (real eval pipeline — see below)
    agents/      →  ~/.config/openai/agents/       (grader subagent prompt)
