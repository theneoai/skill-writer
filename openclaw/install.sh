#!/usr/bin/env bash
# install.sh — Install skill-writer to OpenClaw (~/.openclaw/)
#
# Usage:
#   ./openclaw/install.sh              # install everything
#   ./openclaw/install.sh --dry-run    # preview only, no changes
#
# Installs:
#   ~/.openclaw/skills/skill-writer.md   ← main skill file
#   ~/.openclaw/AGENTS.md                ← agent routing rules (merge)
#   ~/.openclaw/refs/                    ← companion reference files
#   ~/.openclaw/templates/               ← skill creation templates
#   ~/.openclaw/eval/                    ← evaluation rubrics
#   ~/.openclaw/optimize/                ← optimization strategies
...
# ── Create directories ────────────────────────────────────────────────────────
for dir in "$SKILLS_DIR" "$OPENCLAW_HOME/refs" "$OPENCLAW_HOME/templates" \
           "$OPENCLAW_HOME/eval" "$OPENCLAW_HOME/optimize"; do
  if ! $DRY_RUN; then
    mkdir -p "$dir"
  fi
  info "dir: $dir"
...
AGENTS_MD="$OPENCLAW_HOME/AGENTS.md"
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
echo "  ✓ skill-writer installed to OpenClaw"
echo ""
echo "  Paths:"
echo "    Skill:      $SKILL_DST"
echo "    Refs:       $OPENCLAW_HOME/refs/"
echo "    Templates:  $OPENCLAW_HOME/templates/"
echo "    Eval:       $OPENCLAW_HOME/eval/"
echo "    Optimize:   $OPENCLAW_HOME/optimize/"
echo "    AGENTS.md:  $AGENTS_MD"
echo ""
echo "  Next: Restart OpenClaw, then type 'create a skill' to test."
echo ""
