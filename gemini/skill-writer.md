...
<!-- PATH CONVENTION (Claude)
  Companion files are installed to ~/.gemini/ by running: ./claude/install.sh
    refs/        →  ~/.gemini/refs/         (on-demand specs incl. refs/modes/)
    templates/   →  ~/.gemini/templates/
    eval/        →  ~/.gemini/eval/         (incl. trigger-eval.example.json)
    optimize/    →  ~/.gemini/optimize/
    scripts/     →  ~/.gemini/scripts/      (real eval pipeline — see below)
    agents/      →  ~/.gemini/agents/       (grader subagent prompt)
