...
<!-- PATH CONVENTION (Claude)
  Companion files are installed to ~/.hermes/ by running: ./claude/install.sh
    refs/        →  ~/.hermes/refs/         (on-demand specs incl. refs/modes/)
    templates/   →  ~/.hermes/templates/
    eval/        →  ~/.hermes/eval/         (incl. trigger-eval.example.json)
    optimize/    →  ~/.hermes/optimize/
    scripts/     →  ~/.hermes/scripts/      (real eval pipeline — see below)
    agents/      →  ~/.hermes/agents/       (grader subagent prompt)
