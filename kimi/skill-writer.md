...
<!-- PATH CONVENTION (Claude)
  Companion files are installed to ~/.config/kimi/ by running: ./claude/install.sh
    refs/        →  ~/.config/kimi/refs/         (on-demand specs incl. refs/modes/)
    templates/   →  ~/.config/kimi/templates/
    eval/        →  ~/.config/kimi/eval/         (incl. trigger-eval.example.json)
    optimize/    →  ~/.config/kimi/optimize/
    scripts/     →  ~/.config/kimi/scripts/      (real eval pipeline — see below)
    agents/      →  ~/.config/kimi/agents/       (grader subagent prompt)
